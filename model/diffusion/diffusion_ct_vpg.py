...
class CTVPGDiffusion(CTDiffusionModel):
    ...
    def p_mean_var(
        ...
        n_timesteps = self.denoising_steps if not self.use_ddim else self.ddim_steps
        ft_indices = torch.where(index >= (n_timesteps - self.ft_denoising_steps))[0]
        # if self.use_ddim:
        ...
        if len(ft_indices) > 0:
            # print("overwriting noise for fine-tuning steps")
        ...
        # Note that denoised_clip_value is ignored, again
        ...
        score = self.score_from_noise(noise, t)
        # Get mu
        if self.use_ddim:
            """
            μ = √ αₜ₋₁ x₀ + √(1-αₜ₋₁ - σₜ²) ε
            ...
            """
            etas = torch.ones_like(x).to(x.device)  # always one for DDPM

        beta_t = self.get_beta(t)
        dxt = -0.5 * beta_t * x - (0.5 * beta_t * (1 + etas)) * score
        mu = x - dxt * dt
        var = (etas ** 2) * self.get_beta(t) * dt
    ...
    def forward(
        ...
        for i, t in enumerate(t_all):
            ...
            if return_chain:
                if i >= (
                    n_timesteps - self.ft_denoising_steps - 1
                ):
                    chain.append(x)
    ...
    def get_logprobs(
        ...
        indices_single = torch.arange(
            start=n_timesteps - self.ft_denoising_steps,
            end=n_timesteps,
            device=self.device,
        )  # only used for DDIM
    ...
    def get_logprobs_subsample(
        ...
        indices_single = torch.arange(
            start=n_timesteps,
            end=n_timesteps - self.ft_denoising_steps,
            step=-1,
            device=self.device,
        )  # only used for DDIM
        indices = indices_single[denoising_inds]
